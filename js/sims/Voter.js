Voter.resetGlobalVariables = function () {

    Voter.SELECTION = 5;
    Voter.NUM_TURNS = 10;

    Voter.INITIAL_AGENTS = [
        { strategy: "honest" },
        { strategy: "greedy" },
        { strategy: "oneself" },
    ];

    Voter.FLOWER_CONNECTIONS = false;

    PD.NOISE = 0;

};

Voter.resetGlobalVariables();

subscribe("rules/evolution", function (value) {
    Voter.SELECTION = value;
});

subscribe("rules/turns", function (value) {
    Voter.NUM_TURNS = value;
});
...
function Voter(config) {
    ...
    self.populateAgents = function () {
        ...
        for (var i = 0; i < self.agents.length; i++) {
            ...
            var agent = new VoterAgent({ angle: angle, strategy: strategy, tournament: self });
    ...
    self.createNetwork = function () {
        ...
        for (var i = 0; i < self.agents.length; i++) {
            ...
            for (var j = i + 1; j < self.agents.length; j++) {
                ...
                var connection = new VoterConnection({
                    tournament: self,
                    from: playerA,
                    to: playerB,
                    flower_flip: flip
    ...
    self.reset = function () {
        ...
        // Agents & Network...
        AGENTS = JSON.parse(JSON.stringify(Voter.INITIAL_AGENTS));
        self.populateAgents();
        self.createNetwork();
    ...
    self.playOneVoter = function () {
        PD.playOneVoter(self.agents, Voter.NUM_TURNS);
    ...
    var _tick = function (delta) {
        ...
        if (self.STAGE == STAGE_PLAY) {
            /*if(self.isAutoPlaying){
                self.playOneVoter(); // FOR REAL, NOW.
                _playIndex = 0;
                _tweenTimer = 0;
                self.STAGE = STAGE_REST;
        ...
        if (self.STAGE == STAGE_ELIMINATE) {
            self.eliminateBottom(Voter.SELECTION);
            _tweenTimer++;
        ...
        if (self.STAGE == STAGE_REPRODUCE) {
            ...
            // Start
            if (_tweenTimer == 0) {
                self.reproduceTop(Voter.SELECTION);
            }
...
function VoterConnection(config) {
